...
func (x *XTCP) validateInput() error {
	...
	if x.config.Dest != schemeNull && x.config.Dest != schemeStdout && x.config.Dest != schemeStderr {
		...
		switch scheme {
		case schemeUnix, schemeUnixgram, schemeNull, schemeStdout, schemeStderr,
			schemeFile, schemeHTTP, schemeHTTPS, schemeS3Parquet:
