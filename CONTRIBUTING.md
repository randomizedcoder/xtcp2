...
- `oci-xtcp2-start` — load the `oci-xtcp2-min` image into the local Docker daemon and start it (`-dest null`), bind-mounting the host's `/run/netns` and `/run/docker/netns` so the daemon continuously polls the host root namespace plus every container/pod namespace (real sockets). It prints the one-time privileged prerequisites (it never runs `sudo` itself) and refuses to start until the host root netns is attached: `sudo mkdir -p /run/netns && sudo ip netns attach xtcp2host 1`. Extra args after `--` pass through to `xtcp2`. Requires Docker.
- `oci-xtcp2-verify` — confirm the container started by `oci-xtcp2-start` passed its capability check and is serving `xtcp_` metrics on `:9088`, and report how many socket records it has deserialized; exits non-zero on failure. Defined in `nix/runners/` — a parameterized factory (`hostVisibility` toggles the host-mount vs isolated-tmpfs mode), so a runner for another image flavor is a one-line addition there.
